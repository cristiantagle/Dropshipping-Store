...
import { normalizeCategoria, type Slug } from "@/lib/categorias";
...
export default function CategoriaPage({ params }: Props) {
  ...
  const lista = (productos as Producto[])
    .filter(p => normalizeCategoria(p.categoria) === params.slug)
    .slice(0, 10);
    ...
    <div className="space-y-8">
      ...
      {lista.length === 0 ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {lista.map(p => (<ProductCard key={p.id} p={p} onAdd={() => alert("Producto agregado al carro")} />))}
        </div>
      )}
    </div>
  );

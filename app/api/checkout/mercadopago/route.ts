...
/**
 * Crea preferencia en Mercado Pago (SDK v2).
 * Requiere MERCADOPAGO_ACCESS_TOKEN y NEXT_PUBLIC_SITE_URL definidos.
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!init_point) {
      return NextResponse.json(
        { error: "Preferencia creada sin init_point" },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json(
      { error: "Error al crear preferencia", detail: err?.message ?? String(err) },
      { status: 500 }
    );
  }
}

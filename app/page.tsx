...
async function q<T=Producto[]>(fn: (s: NonNullable<ReturnType<typeof supabaseServer>>) => Promise<{data:T|null,error:any}>): Promise<T> {
  const s = supabaseServer();
  if (!s) return [] as unknown as T;
  try {
    const { data, error } = await fn(s);
    if (error) { console.error(error); return [] as unknown as T; }

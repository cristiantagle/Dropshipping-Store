...
// Acepta el builder de Supabase como `any`, hace await y normaliza a T[].
async function exec<T>(builder: any): Promise<T[]> {
  try {
...
export default async function Home() {
  ...
  if (supa) {
    ...
    tendencias = await exec<Producto>(
      supa.from("productos").select("*").eq("destacado", true).order("id", { ascending: true }).limit(6)
    );

    // Top: ventas desc → fallback id asc
    top = await exec<Producto>(

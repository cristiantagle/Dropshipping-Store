...
export type Producto = {
  ...
  nombre: string;
  precio?: number | null;
  // Admitimos ambas variantes para no romper llamadas previas
...
type Props = {
  items: Producto[]; // NO limitamos aquí: la página ya manda 12
};
...
function pickImage(p: Producto): string {
  const raw =
    (p.imagen_url ?? p.imagen ?? (p as any).image ?? "").toString().trim();
  // Si viene con &ixlib/params o tamaños, los respetamos. Si está vacío → fallback
      ...
      {list.map((p) => {
        ...
        const alt =
          (p?.nombre?.toString().trim() ||
            "Producto") + " - imagen de referencia";
        const precio = formatPrice(p?.precio ?? null);

        return (
          <li
            key={p.id}
            className="border rounded-2xl overflow-hidden bg-white hover:shadow transition"
          >
            {/* Usamos <img> simple para evitar problemas de dominios de Next/Image */}
            ...
            <div className="p-3 space-y-2">
              ...
              <div className="pt-2">
                <button
                  ...
                  onClick={() => {
                    // placeholder: integra tu lógica de carrito acá si aplica
                    console.log("Agregar", p.id);
                  }}
                >
                  Agregar
                </button>

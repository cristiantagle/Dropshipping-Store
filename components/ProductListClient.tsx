...
export type Producto = {
  ...
  nombre: string;
  precio?: number | null;
  // posibles llaves para imagen (según tu seed en Supabase)
  imagen?: string | null;
...
export default function ProductListClient({ items }: Props) {
  ...
  return (
    <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      {data.map((p) => {
        let src = pickUrl(p);
        // Caso Bienestar → fallback si viene vacío
        ...
        return (
          ...
          >
            ...
            <p className="text-sm text-gray-600">
              {p.envio || "Envío estándar"}
            </p>

            <div className="mt-2 flex items-center justify-between">
              <span className="font-bold">{fmtCLP(p.precio ?? null)}</span>
              <button
                className="px-3 py-1 rounded-xl bg-lime-600 text-white text-sm hover:bg-lime-700"
                onClick={() => agregar(p)}
              >
                Agregar
              </button>

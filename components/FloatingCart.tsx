...
export default function FloatingCart() {
  ...
  const openCarro = () => (window.location.href = "/carro");

  return (
    <button
      aria-label="Abrir carro"
      onClick={openCarro}
      className="fixed bottom-4 right-4 z-40 w-14 h-14 rounded-full shadow-lg border border-emerald-600/20 transition-transform hover:scale-105 active:scale-95"
      style={{
        background:
          "linear-gradient(180deg, #2bb673 0%, #23a765 100%)",
        WebkitTapHighlightColor: "transparent",
      }}
    >
      {/* Ícono carrito blanco */}
      ...
      {/* Badge único (verde claro sobre borde blanco) */}
      {count > 0 && (
        <span

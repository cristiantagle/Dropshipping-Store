...
export function OptimizedCartProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      logToPersistentLog('info', '📥 [CONTEXT-INIT] Items cargados', {
        count: parsed.length,
        items: parsed.map((i: CartItem) => ({ id: i.id, qty: i.qty })),
      });
      setItems(parsed);
    } catch {
  ...
  useEffect(() => {
    ...
    try {
      ...
      logToPersistentLog('info', '💾 [CONTEXT-BACKUP] Backup automático creado', {
        backupKey,
  ...
  useEffect(() => {
    ...
    if (items.length === 0) {
      ...
      if (currentStoredItems.length > 0) {
        logToPersistentLog(
          'warn',
          '⚠️ [CONTEXT-PROTECTION] Intento de borrado detectado, verificando legitimidad',
        );
        ...
        logToPersistentLog(
          'info',
          '🔄 [CONTEXT-PROTECTION] Rehidratando desde localStorage en lugar de vaciar',
        );
        return;
      }
    }
    ...
    const currentIds = items
      ...
      .sort()
      .join(',');
    const storedIds = currentStoredItems
      .map((i: CartItem) => `${i.id}-${i.qty}`)
      .sort()
      .join(',');
    ...
    logToPersistentLog('info', '💾 [CONTEXT-WRITE] Guardando carrito', {
      totalItems,
  ...
  function add(item: Omit<CartItem, 'qty'>) {
    logToPersistentLog('info', '🛒 [CONTEXT-ADD] Agregando al carrito', {
      id: item.id,
      name: item.name,
    });
    ...
    setItems((prevItems) => {
      ...
      if (found) {
        ...
        logToPersistentLog('info', '📈 [CONTEXT-ADD] Incrementando cantidad', {
          productId: item.id,
          newQty: found.qty + 1,
        });
      } else {
        updatedItems = [...prevItems, { ...item, qty: 1 }];
        logToPersistentLog('info', '🆕 [CONTEXT-ADD] Agregando nuevo producto', {
          productId: item.id,
        });
      }

      logToPersistentLog('info', '📦 [CONTEXT-ADD] Items después', {
        items: updatedItems.map((i) => ({ id: i.id, name: i.name, qty: i.qty })),
      });
      return updatedItems;
    });
  }
  ...
  function increment(id: string) {
    setItems((prev) => prev.map((p) => (p.id === id ? { ...p, qty: Math.min(99, p.qty + 1) } : p)));
  }
  ...
  // Limpiar carrito (intencional)
  function clear() {
    logToPersistentLog('info', '[CONTEXT-CLEAR] Limpieza intencional del carrito');
    try {
      localStorage.setItem('carro', JSON.stringify([]));
      localStorage.setItem('carro_cleared_at', new Date().toISOString());
      Object.keys(localStorage)
        .filter((k) => k.startsWith('carro-backup-'))

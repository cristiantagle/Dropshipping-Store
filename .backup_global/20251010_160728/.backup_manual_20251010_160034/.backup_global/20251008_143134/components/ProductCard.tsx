...
export default function ProductCard({
  id,
  name,
  name_es,        // 👈 nuevo: nombre traducido opcional
  image_url,
  price_cents,
}: {
  id: string;
  name: string;
  name_es?: string;   // 👈 opcional para compatibilidad
  image_url: string;
  price_cents: number;
}) {
    ...
    >
      ...
      <div className="aspect-[4/3] bg-gray-50 rounded-t-xl overflow-hidden">
        <img
          ...
          alt={name_es || name}   // 👈 usamos el traducido si existe
      ...
      <div className="p-4">
        <h3 className="text-base font-semibold text-gray-800 group-hover:text-lime-600 transition-colors line-clamp-2">
          {name_es || name}   {/* 👈 mostramos name_es si está disponible */}
        </h3>

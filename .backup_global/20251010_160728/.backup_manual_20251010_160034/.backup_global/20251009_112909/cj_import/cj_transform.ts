...
export interface CJRawProduct {
  ...
  productImage: string;
  sellPrice: string;
  categoryName: string | null; // categoría original de CJ
}

export interface ProductRow {
  cj_id: string;
  name: string;        // nombre original en inglés
  name_es: string;     // nombre traducido
  productsku: string;
  image_url: string;
  price_cents: number; // precio convertido a CLP
  category_slug: string; // categoría final (hogar, belleza, etc.)
  cj_category: string | null; // 👈 nuevo: categoría original CJ
}
